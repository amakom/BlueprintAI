...
export function ExportMenu({ projectId }: ExportMenuProps) {
  ...
  const buildFullAIPrompt = async (): Promise<string> => {
    ...
    if (!isPro) {
      prompt += `\n---\n`;
      prompt += `> NOTE: This is a basic export (user stories + screens only). Upgrade to BlueprintAI Pro for the full Context Cleaner — including personas, strategy, OKRs, KPIs, competitor analysis, engineering specs, and navigation flows.\n`;
