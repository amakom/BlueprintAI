...
export function ScreenNode({ data, selected }: NodeProps<Node<ScreenNodeData>>) {
  return (
    <div className={`w-72 bg-white rounded-xl border-2 shadow-md transition-all ${
      ...
      {/* Phone Header */}
      <div className="h-8 bg-navy rounded-t-lg flex items-center justify-center relative">
        <div className="w-16 h-4 bg-black rounded-b-lg"></div>
      </div>
      
      {/* Screen Content */}
      <div className="h-96 bg-gray-50 p-4 relative overflow-hidden group">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-gray-300 group-hover:text-gray-400 transition-colors">
            ...
            <span className="text-xs font-medium uppercase tracking-widest">{data.label}</span>
        </div>
